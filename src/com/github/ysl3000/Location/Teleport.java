...
public class Teleport {
	...
	public static void tpm(Player player, String command, String[] args,
			Command cmd) throws Exception {
		if (SmartServerTool.getCommands().getTP(command)
				&& SmartServerTool.getPermission().hasCanTp(player)) {
		...
		} else if (SmartServerTool.getCommands().getTPO(command)
				&& SmartServerTool.getPermission().hasCanTpO(player)) {
			...
			if (args.length == 0) {
				...
				&& SmartServerTool.getPermission().hasCanSwitch(player)) {
		...
		} else if (SmartServerTool.getCommands().getBack(command)) {
	...
	public static void home(Player player, String command, String[] args,
		...
		if (SmartServerTool.getCommands().getHome(command)
				&& SmartServerTool.getPermission().hasHome(player)) {
